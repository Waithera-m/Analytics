...
@Component({
  selector: 'app-ninetyrevenue',
  templateUrl: './ninetyrevenue.component.html',
  styleUrls: ['./ninetyrevenue.component.css']
})
export class NinetyrevenueComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get<any>(url, { headers: headerOptions }).subscribe(data => {
      this.single = data;
      console.log(this.single[0]['revenue']);
      ...
      var text = [];
      for (let i = 0; i < len; i++) {
          text.push(this.single[i]['revenue']);
      ...
      var customers = text;
      this.lineChartData = [{data: customers, label: 'Revenue'},];
